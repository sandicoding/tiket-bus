...
?>
  ...
  window.location = 'admin2.php?halaman=laporan&&bln=<?php echo $bln; ?>&&thn=<?php echo $thn; ?>';
...
$no = 1;
     $query1 = "SELECT tiket.tgl_berangkat, tiket.id_bus, tiket.id_berangkat, tiket.total, tiket.total, bus.id_bus, bus.harga, bus.kelas, keberangkatan.id_berangkat, keberangkatan.tujuan FROM tiket inner join bus on tiket.id_bus = bus.id_bus  inner join keberangkatan on tiket.id_berangkat = keberangkatan.id_berangkat WHERE tiket.id_berangkat = keberangkatan.id_berangkat AND tiket.id_bus = bus.id_bus AND month(tiket.tgl_berangkat) = '$bln' AND year(tiket.tgl_berangkat) = '$thn'";

  /*$query1 = "SELECT * FROM tiket p
